...
const unsigned long AP_RETRY_INTERVAL = 1800000;      // 30 Minuten in AP-Modus bevor neuer Versuch
unsigned long lastWiFiReconnectAttempt = 0;
unsigned long apModeStartTime = 0;
int wifiReconnectAttempts = 0;
const int MAX_WIFI_RECONNECT_ATTEMPTS = 15;
...
void loopMelody()
{
  ...
  if (millis() - noteStartTime >= noteDuration * 1.20)
  { // Pause between notes
    noTone(BUZZER_PIN);
    currentNoteIndex++;
...
void loopWiFi()
{
  ...
  if (millis() - lastWiFiCheckTime > WIFI_CHECK_INTERVAL)
  {
    ...
    // Im AP-Modus: nach 30 Minuten ohne verbundenen Client neuen Verbindungsversuch starten
    if (config.ssid != "" && WiFi.getMode() == WIFI_AP)
    {
      if (WiFi.softAPgetStationNum() == 0 && (millis() - apModeStartTime) > AP_RETRY_INTERVAL)
      {
        Serial.println("AP-Modus: kein Client verbunden, starte neuen WiFi-Verbindungsversuch...");
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
          Serial.println("WiFi verbunden nach AP-Retry!");
          Serial.println(WiFi.localIP());
          wifiConnected = true;
        ...
        {
          Serial.println("Verbindung fehlgeschlagen, starte AP-Modus erneut.");
          WiFi.disconnect();
          WiFi.softAP("Paketkasten-Setup", SOFTAP_PASSWORD);
          apModeStartTime = millis();
        }
    ...
    // Check if we're in station mode (not SoftAP)
    if (config.ssid != "" && WiFi.getMode() != WIFI_AP)
    {
      ...
      if (currentlyConnected != wifiConnected)
      {
        ...
        if (wifiConnected)
        {
          ...
          if (config.mqttServer != "" && !mqttClient.connected())
          {
            Serial.println("Attempting MQTT reconnect after WiFi restore...");
            if (mqttClient.connect("Paketkasten", config.mqttUser.c_str(), config.mqttPassword.c_str()))
            {
      ...
      if (!currentlyConnected)
      {
        if (millis() - lastWiFiReconnectAttempt > WIFI_RECONNECT_INTERVAL)
        {
          ...
          // If too many failed attempts, restart ESP (only when locked and safe)
          if (wifiReconnectAttempts >= MAX_WIFI_RECONNECT_ATTEMPTS)
          {
            Serial.println("Too many WiFi reconnect attempts. Will retry on next cycle.");
            wifiReconnectAttempts = 0; // Reset counter to prevent constant restarts
          }
        }
      }
    }
